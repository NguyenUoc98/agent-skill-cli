...
(async () => {
    ...
    if (!roleArg) {
        ...
        console.error(`  ${colors.dim}Example: npx agent-kit --role=da${colors.reset}`);
        console.error(`  ${colors.info} Supported roles: frontend, backend, po_ba, designer, tester, da\n`);
        process.exit(1);
    }
    ...
    await runSetup(role, currentDir, templateDir, targetDir);
})();

async function runSetup(role, currentDir, templateDir, targetDir) {
        ...
        rolesToInstall.forEach(r => {
            ...
            if (!fs.existsSync(roleSourceDir)) {
                if (!r.includes('/')) {
                    ...
                    console.error(`${colors.pipe} Supported roles: frontend, backend, po_ba, designer, tester, da`);
                    process.exit(1);
                }
                return;
            }
            ...
            // Copy data_dictionary to project root for DA role
            if (r === 'da' || r === 'data') {
