...
### Step 2: Create 2-2-1 Plan and Sync to Jira (Local & Cloud)
...
- Once stored, use `mcp-atlassian` to create a **Sub-task** in the original Jira task for tracking these test plans (use `createJiraIssue` with the parent key).
- Provide the generated set of test scenarios as a description or a comment on this newly created sub-task for cloud visibility.
